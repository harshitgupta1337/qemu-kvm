...
rm -rf %{buildroot}%{_datadir}/%{name}/dtb/pegasos*.dtb
...
rm -rf %{buildroot}%{_datadir}/%{name}/u-boot-sam460.bin
...
# Remove additional unpackaged files
rm -rf %{buildroot}%{_includedir}/qemu-plugin.h
rm -rf %{buildroot}%{_libdir}/%{name}/audio-oss.so
rm -rf %{buildroot}%{_libdir}/%{name}/block-ssh.so
...
%changelog
* Fri Mar 20 2026 Harshit Gupta <guptaharshit@microsoft.com> - 10.1.0-17
- Include mshv-revert-overlapping-mem-mappings.patch for HyperV-Direct backend
